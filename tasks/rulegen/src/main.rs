...
impl RuleConfigOutput {
    ...
    fn extract_output_inner(
        ...
        match element {
            ...
            RuleConfigElement::True | RuleConfigElement::False => Some(("bool".to_string(), None)),
...
struct RuleConfig<'a> {
    ...
    cur_root_path: String,
    semantic: Semantic<'a>,
    definitions: FxHashMap<String, RuleConfigElement>,
    has_errors: bool,
    log_errors: bool,
}
...
impl<'a> RuleConfig<'a> {
    ...
    fn parse_type_array_expression(
        ...
        let elements = array_expression
            ...
            .iter()
            .filter_map(|element| {
                if let ArrayExpressionElement::NullLiteral(_) = element {
                    has_null = true;
                    None
                } else if let ArrayExpressionElement::StringLiteral(string_literal) = element {
    ...
    fn extract_enum_elements(
        ...
        let mut enum_elements = array_expression
            ...
            .filter_map(|arg| {
                ...
                if !inner_has_null && element.is_none() {
                    self.log_error(&format!(
                    ...
                    Expression::ArrayExpression(array_expression) => {
                        ...
                        self.elements = array_expression
                            ...
                            .filter_map(|element| {
                                ...
                                self.cur_root_path = format!("#/items/{element_index}");
                                self.extract_schema_element(object_expression);
    ...
    fn extract_schema_element(&mut self, object_expression: &ObjectExpression<'a>) {
        ...
        // Get definitions/$defs before collecting schema AST
        for object_property_kind in &object_expression.properties {
            let ObjectPropertyKind::ObjectProperty(object_property) = &object_property_kind else {
                self.log_error(&format!(
                    "Cannot parse object property kind: {}",
            ...
            match identifier.name.as_str() {
                "$defs" | "definitions" => {
                    ...
                    self.definitions = self
                        .extract_properties(object_expression)
                        .into_iter()
                        .map(|(key, value)| {
                            (format!("{}/{}/{}", self.cur_root_path, identifier.name, key), value)
                        })
                        .collect();
                }
                _ => {}
            ...
            match identifier.name.as_str() {
                "$ref" => {
                    let Expression::StringLiteral(string_literal) = &object_property.value else {
                        self.log_error(&format!(
                            "Cannot parse `$ref` value: {}",
                            object_property.value.span().source_text(self.semantic.source_text())
                        ));
                        continue;
                    };
                    let reference = string_literal.value.as_str();
                    if let Some(element) = self.definitions.get(reference) {
                        rule_config_element = Some(element.clone());
                    } else {
                        self.log_error(&format!(
                            "Cannot parse `$ref` value: {}",
                            object_property.value.span().source_text(self.semantic.source_text())
                ...
                "anyOf" | "oneOf" => {
                    if rule_config_element.is_some() {
                        self.log_error(&format!(
                            "Cannot parse `{}` value with other properties, ignoring",
                            identifier.name
                        ));
                        // continue;
                    }
        ...
        Ok(Ok(body)) => {
            ...
            let config_names = config
                ...
                .filter_map(|(index, element)| {
                    ...
                    let element_name = format!("{pascal_rule_name}{display_index}");
